...
function keyMove(car) {
    ...
    socket.on('midicar', (note) => {
        console.log('midi note received: ' + note);
        switch (note) {
            case "la":  
                inputMap["ArrowUp"] = true;
                inputMap["ArrowDown"] = false;
                break;  
            case "do":
                inputMap["ArrowUp"] = false;
                inputMap["ArrowDown"] = true;    
                break;
            case "fa":
                inputMap["ArrowUp"] = false;
                inputMap["ArrowDown"] = false;
                inputMap["ArrowLeft"] = false;
                inputMap["ArrowRight"] = false;        
                break;
            case "re":
                inputMap["ArrowRight"] = true;
                setTimeout(() => {
                    inputMap["ArrowRight"] = false;
                }, 100);    
                break;
            case "sol":
                inputMap["ArrowLeft"] = true;
                setTimeout(() => {
                    inputMap["ArrowLeft"] = false;
                }, 100);     
                break;
        }
        //var noteToSend = { note: notesMap.get(note) };
    });
